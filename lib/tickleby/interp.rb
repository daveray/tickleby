...
module Tickleby
  ...
  class Frame
    ...
    def initialize(interp, parent = nil)
      @inter = interp
      @parent = parent
      @variables = {}
    end

    def map_variable(name, frame)
      @variables[name] = frame unless self == frame
    end
    ...
    def get_variable(name)
      ...
      if value.respond_to? :get_variable
  ...
  class Interp
    ...
    def initialize
      @commands = { 
        "set" => SetCommand.new,
        "puts" => PutsCommand.new,
        "proc" => ProcCommand.new,
        "return" => ReturnCommand.new,
        "global" => GlobalCommand.new
      }

      @stack = [Frame.new(self, nil)]
      @return_flag = nil
    end
    ...
    def get_global(name)
      get_global_frame.get_variable(name)
    end
    ...
    def eval_command(words)
      ...
      if !command
        raise "Unknown command '#{name}', #{words}"
      end
    ...
    def with_new_frame
      @stack.push Frame.new(self, @stack[-1])
      begin
        yield
      ensure
        @stack.pop
